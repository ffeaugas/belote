...
export type GameAction =
    ...
    | { type: 'PLAYER_SIT'; playerId: string; position: PlayerPosition; timestamp: number }
    ...
    | { type: 'TRICK_WON'; winnerPosition: PlayerPosition; points: number; timestamp: number }
    | { type: 'ROUND_END'; score: RoundScore; timestamp: number }
    | { type: 'GAME_END'; winnerTeam: Team; finalScore: GameScore['total']; timestamp: number }
    | { type: 'PLAYER_DISCONNECT'; playerId: string; timestamp: number }
    | { type: 'PLAYER_RECONNECT'; playerId: string; timestamp: number }
    | { type: 'TIMEOUT'; playerId: string; autoAction: LegalAction; timestamp: number };
